...
from produit.filtre import ArticleFiltre, CommandeFiltre
...
class Panier2(View):
    def get(self, request):
        ...
        '''for x2 in my_orders:
            ...
            #art=Article2.objects.using('user').get(des_art = x2.des_art) 
    ...
    def post(self, request):
        ...
        print(request.POST )
        ma = (request.POST.getlist('des_art'))
        ...
        for item in range(len(ma)):
            print(ma[item])
            
            ww= Temp.objects.using('user').filter(des_art=ma[item],user_name=c).first()
        ...
        for x in my_orders:
            ...
            c=  request.session['username'] 
            com = Commande.objects.using('user').create(quantité=b,COD_ART=a,user_name=c,prix_vente_ttc=p)
            com.save()
...
class commande(View):
    def get(self, request):
        ...
        return render(request,'commande.html',{'cc':my_orders,'aa':my_orders2})
    
   
    
    def get2(self,request  ,*args, **kwargs):
        rahma=Commande.objects.using('user').all()
        template_name = 'commande.html'
        
        #f= Article2.object.get('code famille','DES_ART')
        '''f = CommandeFiltre(request.GET, queryset=Commande.objects.using('user').all())    
        my_orders = Temp.objects.using('user').filter(user_name=request.session['username'])   
        q2= (Commande.objects.using('user').filter(Q(date__contains=request.GET['date'])))
        print('qqqqqqqqqqq',q2)
        return render (request,template_name ,{'rahma': rahma, 'filter': f,'q2': q2,'aa':my_orders})'''
...
class achat(View):
    ...
    def post(self,request,*args, **kwargs):
        ...
        a=  request.POST.get('des_art')
        v=  request.POST.get('cod_art')
        ...
        try :
            rahma=Temp.objects.using('user').all() 
            rahmaa= Temp.objects.using('user').filter(des_art=a,user_name=c).first()
           ...
           art= rahma=Temp.objects.using('user').all() 
           #print(art.prix_vente_ttc)
        

           rahmaa= Temp.objects.using('user').create(des_art=a,quantité=b,COD_ART=v,user_name=c,date=d)
           rahmaa.save()
